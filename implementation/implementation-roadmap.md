...
| 多 SAEU executor isolation | `in_progress` | backend Executor Registry 与 per-run qwen serve process 已落地；container worker command-template 已预留，仍需实机 Docker/资源隔离验收 |
...
- `container` 作为 command-template strategy 预留；下一步应在 VPS 上接 Docker
  cgroup/memory/pids/network 限制并补 E2E 验收。
